...
RSpec.describe Api::V1::PicturesController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    context 'with bad attributes ' do
      it 'album responds with status unprocessable_entity' do
  ...
  describe 'PATCH #update' do
    ...
    context 'invalid attributes' do
      ...
      it 'shouldn\'t update picture with oversized file' do
        patch :update, {id: @picture.id, picture: attributes_for(:picture, :oversized_file)}, format: :json
        expect(response).to have_http_status(:unprocessable_entity)
        expect(assigns(:picture).image.size).to be > 512_000
      end
    end
  end
